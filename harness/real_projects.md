...
Build each with `CC=$PWD/target/release/acc`.
...
- **sqlite amalgamation**: PASS — `third_party/stage_c/sqlite/sqlite3.c` under acc → link → open :memory: SUM 40+2=42
...
|---|---------|------------------------|-----------------------------|
| 1 | **SQLite** | Official **`testfixture`** + **`test/veryquick.test`** under acc (suite summary in log; nfail=0 or documented ledger skips only) | `sqlite_reg` 38/38, amalgamation `sqlite ok sum=42` |
